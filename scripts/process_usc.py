...
class USCProcessor:
    ...
    def __init__(self, data_dir: str = "./data/xml", use_contextualized: bool = None):
        ...
        if use_contextualized is None:
            use_contextualized = os.getenv("USE_CONTEXTUALIZED_EMBEDDINGS", "true").lower() == "true"
    ...
    async def process_file(self, filepath: Path) -> int:
        ...
        with Progress(
            ...
            if self.use_contextualized:
                ...
                console.print(f"[green]Created {len(document_chunks)} documents with {total_chunks} total chunks[/green]")
                ...
                # Estimate cost
                avg_chunk_size = sum(len(chunk['text'].split()) for doc in document_chunks for chunk in doc) // total_chunks if total_chunks else 0
                cost_estimate = self.embedder.estimate_cost(total_chunks, avg_chunk_size)
                console.print(f"[yellow]Estimated embedding cost: ${cost_estimate['embedding_cost']:.4f} using {cost_estimate['model']}[/yellow]")
                ...
                console.print(f"[green]Generated embeddings for {len(embedding_results)} documents[/green]")
...
@click.option("--use-context/--no-context", default=None, help="Use contextualized embeddings (voyage-context-3)")
@click.option("--chunk-strategy", type=click.Choice(['hierarchical', 'chapter', 'section']), 
              default=None, help="Chunking strategy for contextualized embeddings")
def main(title, process_all, clear, data_dir, use_context, chunk_strategy):
    ...
    console.print(f"Embedding mode: {'Contextualized (voyage-context-3)' if processor.use_contextualized else 'Standard (voyage-law-2)'}")
