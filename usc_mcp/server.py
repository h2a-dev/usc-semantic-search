...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(log_file), logging.StreamHandler()],  # Also log to stderr
)
...
async def search_usc(
    ...
    try:
        ...
        return [{"error": str(e), "message": "Failed to search USC. Please try again."}]
...
async def get_citation(citation: str) -> Optional[dict]:
    ...
    try:
        ...
        return {"error": str(e), "message": f"Failed to retrieve citation: {citation}"}
...
async def browse_usc(
    ...
    try:
        ...
        return {"error": str(e), "level": "error", "items": []}


@mcp.tool
async def get_context(section_id: str, context_size: int = 2) -> List[dict]:
    """
    ...
    try:
        ...
        return [{"error": str(e), "message": f"Failed to get context for section: {section_id}"}]
    ...
    try:
        ...
        return {
            ...
            "embedding_configuration": {
                ...
                "context_dimension": (
                    embedder.context_dimension if embedder.use_contextualized else None
                ),
