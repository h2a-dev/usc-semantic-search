...
class ChromaDatabase:
    ...
    def __init__(self, 
                 ...
                 collection_name: str = "usc_sections"):
        """
        ...
        default_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "db")
    ...
    def add_embeddings(self, embedding_results: List[Any]) -> int:
        ...
        for result in embedding_results:
            ...
            documents.append(f"{result.metadata.get('heading', '')} {result.metadata.get('full_citation', '')}")
    ...
    def get_by_id(self, section_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            results = self.collection.get(
                ids=[section_id],
                include=['metadatas', 'documents']  # Don't include embeddings to avoid numpy issues
            )
    ...
    def browse_hierarchy(self, 
                        ...
                        chapter_num: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        results = self.collection.get(
            where=where,
            include=['metadatas'],
            limit=1000  # Get all matching sections
        )
